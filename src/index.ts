...
export default {
	...
	async scheduled(event, env, ctx): Promise<void> {
		...
		const currentIds = buildRoomIds(current);
		const previousIds = await getSnapshot(env);

		const newIds = compareRooms(currentIds, previousIds);
		...
		if (newIds.length > 0 || currentIds.length !== previousIds.length) {
			await updateSnapshot(env, currentIds);
		}
	},
